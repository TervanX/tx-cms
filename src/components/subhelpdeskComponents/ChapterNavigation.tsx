          ...
          {navigationItems.map((item) => {
            ...
            return (
              <li key={item.id} className="flex-1">
                <a
                  className={`flex w-full justify-center px-3 py-[0.875rem] font-sans text-xs/none tracking-[1.2px] whitespace-nowrap uppercase hover:text-black md:justify-start md:px-0 md:whitespace-normal dark:hover:text-white ${
                    isActive ? "text-black" : "text-dark/40"
                  }`}
                ...
                >
                  <span
                    className={`mr-2 hidden size-[10px] rounded-full bg-black md:inline-block ${
                ...
                <span className="relative block h-[1px] w-full bg-dark/20">
                  <motion.span
                    className={`absolute top-0 left-0 lg:h-[1px] h-[3px] w-full origin-left ${
                      isActive ? "opacity-100" : "opacity-0"
                    } bg-black`}
                    style={{
                      scaleX: progress,
