...
export default function Header() {
  ...
  const backgroundColor = isWaitlistPage
    ? "bg-white"
  ...
  const hoverTextColor = isWaitlistPage
    ? "hover:text-black/80"
    : "hover:text-white/80";
  ...
  return (
    <header className="fixed top-0 right-0 left-0 z-50">
      <nav
        className={`flex w-full border-b ${borderColor} ${backgroundColor} px-3 py-3 font-semibold tracking-tight backdrop-blur-[58px] md:border-0 md:py-2`}
      >
        ...
        {!isWaitlistPage && (
          <ul className="ml-auto flex items-center space-x-2 md:flex md:items-center md:space-x-4 md:py-2.5">
            ...
            <Link href="/contact/request-access" className="group relative">
              <button
                className={`relative inline-block cursor-pointer rounded-md font-semibold tracking-tight whitespace-nowrap text-base px-4 py-2.5 leading-none overflow-hidden`}
              >
                <span
                  className={`absolute inset-0 block w-full rounded-md transition-all duration-400 bg-primary group-hover:bg-primary/80`}
                ></span>
                <span className={`relative z-10 ${buttonTextColor}`}>
                  Get Started
                </span>
              </button>
            </Link>
