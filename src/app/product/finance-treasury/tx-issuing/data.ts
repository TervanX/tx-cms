export const productivityProps = {
    heading: {
        main: "Autonomous token ",
        highlighted: "lifecycle management",
    },
    description:
        "TX Issuing automates the most complex aspects of digital asset creation—from smart contract deployment to regulatory compliance—executing with precision across all supported chains.",
    images: {
        mobile: {
            src: "/assets/productivity-hero-mobile.webp",
            alt: "Illustration of productive bees in a grassy meadow",
        },
        desktop: {
            src: "/assets/solution1.jpg",
            alt: "Illustration of productive bees in a grassy meadow",
        },
        product: {
            src: "/assets/productivity-1-desktop.webp",
            alt: "Helpdesk product UI",
        },
    },
    features: [
        {
            title: 'AI-Optimized Chain Selection',
            description: 'Our system analyzes your tokenomics, target audience, and compliance requirements to recommend the optimal blockchain for deployment. The AI evaluates gas costs, network security, and regulatory clarity to ensure your asset launches on the best possible foundation.'
        },
        {
            title: 'Automated Compliance Engine',
            description: 'Intelligently embed regulatory requirements directly into your token\'s smart contract. The system automatically applies transfer restrictions, whitelisting, and jurisdictional controls based on your selected compliance framework.'
        },
        {
            title: 'Smart Contract Factory',
            description: 'Deploy audited, secure token contracts with customizable parameters. Our library of vetted smart contract templates reduces development time while ensuring enterprise-grade security and functionality.'
        }
    ],
    testimonial: {
        quote:
            "TX Issuing enabled us to launch our security token across 8 jurisdictions in 72 hours instead of 6 months. The compliance automation alone saved us $500,000 in legal fees.",
        author: "Maria Rodriguez",
        role: "Head of Digital Assets at Global Investment Bank",
        companyLogo: ` <svg
        xmlns="http://www.w3.org/2000/svg"
        width="102"
        height="25"
        fill="none"
      >
        {/* Lightspeed logo SVG */}
        <g clipPath="url(#lightspeed_svg__a)">
          <mask
            id="lightspeed_svg__b"
            width="102"
            height="25"
            x="0"
            y="0"
            maskUnits="userSpaceOnUse"
            style={{ maskType: "luminance" }}
          >
            <path fill="#fff" d="M102 .9H0v23.926h102z" />
          </mask>
          <g fill="#191513" mask="url(#lightspeed_svg__b)">
            {/* Logo paths would go here */}
          </g>
        </g>
      </svg>`,
        image: {
            src: "/assets/lightspeed-headshot-min.png",
            alt: "Angelo Livanos",
        },
    },
};

export const usabilityProps = {
    heading: {
        main: "Enterprise-grade tokenization ",
        highlighted: " made accessible",
    },
    description:
        "An intuitive platform for managing the complete digital asset lifecycle—from initial creation to ongoing management and compliance across multiple chains.",
    images: {
        mobile: {
            src: "/assets/usability-hero-mobile.webp",
            alt: "Illustration of people walking down a flowery path",
        },
        desktop: {
            src: "/assets/usability-hero-desktop.webp",
            alt: "Illustration of people walking down a flowery path",
        },
        mainFeature: {
            src: "/assets/usability-1-desktop.webp",
            alt: "Helpdesk usability instant insight product visual",
        },
    },
    usabilityFeatures: [
        {
            title: 'Unified Issuance Dashboard',
            description: 'Monitor all issued assets, holder distributions, and compliance status from a single interface. Track minting, burning, transfers, and regulatory updates across your entire token portfolio',
        }
    ],
    additionalFeatures: [
        {
            image: { src: "/assets/usability-2-desktop.webp" },
            title: 'Visual Token Designer',
            description: 'Create and configure digital assets without Solidity expertise. Use our drag-and-drop interface to define tokenomics, governance rules, and compliance parameters through an intuitive visual builder',
        },
        {
            image: { src: "/assets/usability-2-desktop.webp" },
            title: 'Pre-Built Token Templates',
            description: 'Accelerate deployment with industry-specific templates:',
            list: {
                items: [
                    'Security Tokens with regulatory compliance',
                    'Utility Tokens for ecosystem applications',
                    'Stablecoins with collateral management',
                    'Governance Tokens with voting mechanisms',
                ]
            }
        },
        {
            image: { src: "/assets/usability-2-desktop.webp" },
            title: 'Comprehensive Ecosystem Integration',
            description: 'Connect TX Issuing to your entire digital asset stack:',
            list: {
                items: [
                    'KYC/AML Providers for identity verification',
                    'Secondary Markets for exchange listings',
                    'Governance Platforms for token voting',
                    'Reporting Systems for regulatory compliance',
                ]
            }
        },
    ],
    sectionStyle: {
        backgroundColor: "var(--color-cream)",
    },
};

export const outboundProps = {
    heading: {
        main: "Intelligent issuance analytics ",
        highlighted: "that drive better decisions",
    },
    description:
        "Move beyond basic token metrics to predictive insights that optimize asset performance and compliance posture.",
    images: {
        mobile: {
            src: "/assets/outbound-hero-mobile.webp",
            alt: "Illustration of a futuristic satellite receiver in a rural setting",
        },
        desktop: {
            src: "/assets/outbound-hero-desktop.webp",
            alt: "Illustration of a futuristic satellite receiver in a rural setting",
        },
        mainFeature: {
            src: "/assets/outbound-1-desktop.webp",
            alt: "Helpdesk outbound product visual",
        },
    },
    outboundFeatures: [
        {
            title: 'Real-Time Holder Analytics',
            description: `Monitor token distribution, holder behavior, and transfer patterns across all chains. Track concentration risks, liquidity metrics, and adoption trends with institutional-grade precision.`
        },
        {
            title: 'Predictive Compliance Monitoring',
            description: 'Our AI models analyze regulatory updates, enforcement actions, and jurisdictional changes to forecast compliance requirements. Receive proactive alerts for emerging regulatory risks before they impact your assets.'
        },
        {
            title: 'Smart Liquidity Intelligence',
            description: 'Automatically identify optimal liquidity venues and market-making opportunities. The system analyzes trading patterns, venue reliability, and market depth to recommend listing strategies and liquidity provisioning.'
        },
        {
            title: 'Token Performance Benchmarking',
            description: 'Compare your asset\'s performance against similar tokens across multiple metrics—adoption, liquidity, compliance, and technical performance. Identify optimization opportunities and best practices.'
        }

    ],
    g2Comparison: {
        title: "TX Issuing delivers enterprise-grade tokenization capabilities",
        description:
            "",
        items: [
            { name: ' supported blockchains for deployment', value: '300+', color: 'text-black' },
            { name: ' integrated regulatory frameworks', value: '50+', color: 'text-black/40' },
            { name: ' smart contract deployment success rate', value: '99.9%', color: 'text-black/40' },
            { name: ' average token creation time', value: '<5m', color: 'text-black/40' }
        ],
    },
    sectionStyle: {
        backgroundColor: "white",
    },
};

export const featuresProps = {
    title: "Features",
    features: [
        {
            id: 'multi-chain-token-deployment',
            name: 'Multi-Chain Token Deployment',
            href: '/suite/issuing/multi-chain-deployment',
            description: 'Launch identical tokens across multiple blockchains with single deployment.',
            image: '/assets/photo-1552664730-d307ca884978.jpg'
        },
        {
            id: 'ai-optimized-chain-selection',
            name: 'AI-Optimized Chain Selection',
            href: '/suite/issuing/chain-selection',
            description: 'Intelligently recommend optimal blockchains based on use case and requirements.',
            image: '/assets/photo-1677442136019-21780ecad995.jpg'
        },
        {
            id: 'automated-compliance-embedding',
            name: 'Automated Compliance Embedding',
            href: '/suite/issuing/compliance-embedding',
            description: 'Apply jurisdictional rules and transfer restrictions directly in smart contracts.',
            image: '/assets/photo-1552664730-d307ca884978.jpg'
        },
        {
            id: 'smart-contract-factory',
            name: 'Smart Contract Factory',
            href: '/suite/issuing/contract-factory',
            description: 'Access library of audited, customizable token contract templates.',
            image: '/assets/photo-1677442136019-21780ecad995.jpg'
        },
        {
            id: 'real-time-holder-management',
            name: 'Real-Time Holder Management',
            href: '/suite/issuing/holder-management',
            description: 'Monitor and manage token holders across all deployed chains.',
            image: '/assets/photo-1552664730-d307ca884978.jpg'
        },
        {
            id: 'dynamic-tokenomics-engine',
            name: 'Dynamic Tokenomics Engine',
            href: '/suite/issuing/tokenomics-engine',
            description: 'Configure minting, burning, and distribution rules with visual tools.',
            image: '/assets/photo-1677442136019-21780ecad995.jpg'
        },
        {
            id: 'integrated-kyc-aml',
            name: 'Integrated KYC/AML',
            href: '/suite/issuing/kyc-aml',
            description: 'Seamlessly connect identity verification providers during token issuance.',
            image: '/assets/photo-1552664730-d307ca884978.jpg'
        },
        {
            id: 'secondary-market-preparation',
            name: 'Secondary Market Preparation',
            href: '/suite/issuing/market-preparation',
            description: 'Automatically generate materials for exchange listings and liquidity provisioning.',
            image: '/assets/photo-1677442136019-21780ecad995.jpg'
        },
        {
            id: 'governance-module-setup',
            name: 'Governance Module Setup',
            href: '/suite/issuing/governance-setup',
            description: 'Configure voting mechanisms and governance parameters for token holders.',
            image: '/assets/photo-1552664730-d307ca884978.jpg'
        },
        {
            id: 'cross-chain-bridging',
            name: 'Cross-Chain Bridging',
            href: '/suite/issuing/cross-chain-bridging',
            description: 'Enable seamless token movement between different blockchain networks.',
            image: '/assets/photo-1677442136019-21780ecad995.jpg'
        },
        {
            id: 'regulatory-reporting',
            name: 'Regulatory Reporting',
            href: '/suite/issuing/regulatory-reporting',
            description: 'Generate compliant reports for multiple jurisdictions automatically.',
            image: '/assets/photo-1552664730-d307ca884978.jpg'
        },
        {
            id: 'token-performance-analytics',
            name: 'Token Performance Analytics',
            href: '/suite/issuing/performance-analytics',
            description: 'Track adoption, liquidity, and compliance metrics across all deployments.',
            image: '/assets/photo-1677442136019-21780ecad995.jpg'
        }
    ],
    defaultActiveFeature: "omnichannel",
    sectionStyle: {
        backgroundColor: "var(--color-cream)",
    },
    linkStyle: {
        text: "Find out more",
    },
};